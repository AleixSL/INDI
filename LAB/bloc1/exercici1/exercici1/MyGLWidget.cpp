...
void MyGLWidget::initializeGL ()
{
  ...
  // Activar el canal de transparència
//  glEnable(GL_BLEND);
...
void MyGLWidget::pintaMuntanyes(){
    ...
    transformacioMuntanya(1.0, this->xSegonaMuntanya, 1.0);
    glBindVertexArray(VAO_MUNTANYA);
    glDrawArrays(GL_TRIANGLE_FAN, 0, MUNTANYA_NUM_VERTEXS);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
    ...
    switch (event->key()) {
        case Qt::Key_A: {
            if(this->anglePalanca < M_PI/6) {
                this->anglePalanca += M_PI/36;
            }
            break;
        }
        case Qt::Key_D: {
...
void MyGLWidget::transformacioMuntanya(double h, double xPos, float transparencia){
        ...
        glm::mat4 TG (1.0);
        if (h == 0.7) {
            TG = glm::scale(TG, glm::vec3(-xPos/2, h, 0.0));
            TG = glm::translate(TG, glm::vec3(xPos, xPos+0.55, 0.0));
        }
        else if(h == 1.0) {
            TG = glm::scale(TG, glm::vec3(h/2, h/2, 0.0));
            TG = glm::translate(TG, glm::vec3(0.0, xPos*2, 0.0));
        }

//        float transp = transparencia;
...
void MyGLWidget::transformacioGronxador(){
    ...
    TG = glm::rotate(TG, this->anglePalanca, glm::vec3(0.0, 0.0, 1.0));
    TG = glm::scale(TG, glm::vec3(0.8, 0.1, 0.0));
...
void MyGLWidget::transformacioPes(int left){
    ...
    glm::mat4 TG (1.0f);
    float rotacio = M_PI + this->anglePalanca;
    if(left == 1) {
        TG = glm::translate(TG, glm::vec3(0.0, -0.75, 0.0));
        TG = glm::rotate(TG, rotacio, glm::vec3(0.0, 0.0, 1.0));
        TG = glm::translate(TG, glm::vec3(-0.35, -0.1, 0.0));
        TG = glm::scale(TG, glm::vec3(0.1, 0.1, 0.0));
    }
    else {
        TG = glm::translate(TG, glm::vec3(0.0, -0.75, 0.0));
        TG = glm::rotate(TG, rotacio, glm::vec3(0.0, 0.0, 1.0));
...
void MyGLWidget::creaBufferMuntanya(){
    ...
    glm::vec3 Colors[MUNTANYA_NUM_VERTEXS]= {
        glm::vec3(155/255.0, 121/255.0, 61/255.0),
        glm::vec3(155/255.0, 121/255.0, 61/255.0),
        glm::vec3(155/255.0, 121/255.0, 61/255.0),
        glm::vec3(255/255.0, 255/255.0, 255/255.0),
        glm::vec3(255/255.0, 255/255.0, 255/255.0),
        glm::vec3(255/255.0, 255/255.0, 255/255.0),
        glm::vec3(255/255.0, 255/255.0, 255/255.0),
        glm::vec3(255/255.0, 255/255.0, 255/255.0)
