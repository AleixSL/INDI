...
void MyGLWidget::iniEscena ()
{
  ...
  centreEsc = glm::vec3 (0.0, 0.0, 0.0);
  radiEsc = 15;
}
...
void MyGLWidget::iniFocus() {
    ...
    posF1 = glm::vec3(0.0, 5.0, -10.0);
    glUniform3fv(posFocusLoc[0], 1, &posF1[0]);
    glUniform3fv(colFocusLoc, 1, &colF[0]);

    // Focus 2
    posF2 = glm::vec3(0.0, 5.0, 0.0);
    glUniform3fv(posFocusLoc[1], 1, &posF2[0]);

    // Focus 3
    posF3 = glm::vec3(0.0, 5.0, 10.0);
    glUniform3fv(posFocusLoc[2], 1, &posF3[0]);

    // Número de focus inicials
    numeroFocus = 3;
    glUniform1i(numFocusLoc, numeroFocus);
}

void MyGLWidget::iniReactor() {
    colR = glm::vec3(1.0, 0.2, 0.0);
    posR = glm::vec3(-1.9, 3.60, -5.65);
    glUniform3fv(posReactorLoc, 1, &posR[0]);
    glUniform3fv(colReactorLoc, 1, &colR[0]);
}
...
void MyGLWidget::modelTransformAvio ()
{
  ...
  glUniformMatrix4fv(TGAvioLoc, 1, GL_FALSE, &TG[0][0]);
  glUniformMatrix4fv(transLoc, 1, GL_FALSE, &TG[0][0]);
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)  // Cal modificar aquesta funció...
{
    ...
    switch (event->key()) {
        ...
        case Qt::Key_E: {
            if(colR.x == 0.0) {
                colR = glm::vec3(1.0, 0.2, 0.0);
                emit setReactor(true);
            }
            else {
                colR = glm::vec3(0.0, 0.0, 0.0);
                emit setReactor(false);
            }
            glUniform3fv(colReactorLoc, 1, &colR[0]);
            break;
        }
...
void MyGLWidget::canviaNumFocus(int f) {
    makeCurrent();
    numeroFocus = f;
    glUniform1i(numFocusLoc, numeroFocus);
    update();
...
void MyGLWidget::canviaReactor(bool b) {
    makeCurrent();
    if(b == true) {
        colR = glm::vec3(1.0, 0.2, 0.0);
    }
...
void MyGLWidget::carregaShaders()
{
  ...
  posFocusLoc[0] = glGetUniformLocation(program->programId(), "posFocus[0]");
  posFocusLoc[1] = glGetUniformLocation(program->programId(), "posFocus[1]");
  posFocusLoc[2] = glGetUniformLocation(program->programId(), "posFocus[2]");
  colFocusLoc = glGetUniformLocation(program->programId(), "colFocus");
  numFocusLoc = glGetUniformLocation(program->programId(), "numFocus");
  posReactorLoc = glGetUniformLocation(program->programId(), "posReactor");
  colReactorLoc = glGetUniformLocation(program->programId(), "colReactor");
  TGAvioLoc = glGetUniformLocation (program->programId(), "TGAvio");
}
