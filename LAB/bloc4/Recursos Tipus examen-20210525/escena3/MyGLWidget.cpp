...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(posXpat1, -1.0+0.5, 0.0));
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(2.0, -1.0+1.25, 1.0));
  TG = glm::rotate(TG, angleModels, glm::vec3(0.0, 1.0, 0.0));
  TG = glm::rotate(TG, -float(M_PI/2), glm::vec3(0.0, 1.0, 0.0));
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva and !camPat1)
    Proj = glm::perspective(FOV, rav, znear, zfar);
  else if(camPat1) {
      Proj = glm::perspective(float(M_PI/2.0), rav, float(0.1), 3.0f*radiEsc);
  }
  else if (!perspectiva and !camPat1)
...
void MyGLWidget::viewTransform ()
{
  ...
  if (camPat1 == false) {
      glm::mat4 ViewAux1(1.f);
      ViewAux1 = glm::translate(ViewAux1, glm::vec3(0, 0, -2*radiEsc));
      ViewAux1 = glm::rotate(ViewAux1, -angleX, glm::vec3(1, 0, 0));
      ViewAux1 = glm::rotate(ViewAux1, -angleY, glm::vec3(0, 1, 0));
      View = ViewAux1;
  }
  else {
      glm::mat4 ViewAux2;
      ViewAux2 = glm::lookAt(OBS, VRP, UP);
      View = ViewAux2;
  }
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_A: {
      posXpat1 -= 0.1;
      OBS.x -= 0.1;
      glUniform1f(posXpat1Loc, posXpat1);
      modelTransformPatricio();
      viewTransform();
      break;
    }
...
void MyGLWidget::canviaColorR(int color) {
    makeCurrent();
...
void MyGLWidget::canviaColorG(int color){
    makeCurrent();
    float aux = color/100.0f;
    diff.y = aux;
    createBuffersTerraIParet();
...
void MyGLWidget::canviaColorB(int color){
    makeCurrent();
    float aux = color/100.0f;
    diff.z = aux;
    createBuffersTerraIParet();
...
void MyGLWidget::carregaShaders()
{
  ...
  posXpat1Loc = glGetUniformLocation(program->programId(), "posXpat1");
}
