...
//  glViewport (0, 0, ample, alt);
  ...
  // -------------- LEGOMEN -----------------
  if(!legoman1Tocat) {
      // Activem el VAO per a pintar el Legoman 1
  ...
  if(legoman1Tocat) {
      // Activem el VAO per a pintar el Legoman 1
      glBindVertexArray (VAO_LegoAss);

      modelTransformLegoman1 ();
      // Pintem Legoman 1
      glDrawArrays(GL_TRIANGLES, 0, legoAss.faces().size()*3);
  }

  if(!legoman2Tocat) {
      // Activem el VAO per a pintar el Legoman 2
      glBindVertexArray (VAO_Lego);
...
void MyGLWidget::modelTransformLegoman1 ()
{
  ...
  TG = glm::translate(TG, glm::vec3(-7., 0., 3.));
  TG = glm::scale(TG, glm::vec3(escalaModel, escalaModel, escalaModel)*3.f);
...
void MyGLWidget::modelTransformLegoman1Ass ()
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(-7., 0., 3.));
  TG = glm::scale(TG, glm::vec3(escalaModelAss, escalaModelAss, escalaModelAss)*3.f);
  TG = glm::rotate(TG, float(M_PI/2.0), glm::vec3(0., 1., 0.));
  TG = glm::translate(TG, -centreBaseModelAss);

  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
}

void MyGLWidget::modelTransformLegoman2Ass ()
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(-7., 0., -3.));
  TG = glm::scale(TG, glm::vec3(escalaModelAss, escalaModelAss, escalaModelAss)*3.f);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  makeCurrent();
  switch (event->key()) {
    case Qt::Key_Left: { // moviment pilota
      if((posPilota.z+0.5) < 6.9) {
          posPilota.z += 0.5;
      }
      modelTransformPilota();
      break;
    }
    case Qt::Key_Right: { // moviment pilota
      if((posPilota.z-0.5) > -6.9) {
          posPilota.z -= 0.5;
      }
      modelTransformPilota();
    ...
    case Qt::Key_Up: { // moviment pilota
      if((posPilota.x-0.5) > -9.9) {
          posPilota.x -= 0.5;
      }
      modelTransformPilota();
      break;
    }
    case Qt::Key_Down: { // moviment pilota
      if((posPilota.x+0.5) < 9.9) {
        posPilota.x += 0.5;
      }
      modelTransformPilota();
  ...
  if((posPilota.x == -7.0) and (posPilota.z == 3.0)) {
      legoman1Tocat = true;
      modelTransformLegoman1Ass();
  }
  if((posPilota.x == -7.0) and (posPilota.z == -3.0)) {
      legoman2Tocat = true;
      modelTransformLegoman2Ass();
  }
  update();
}
...
void MyGLWidget::creaBuffersLegomanAssegut()
{
    creaBuffersModel(legoAss, "./models/legoman-assegut.obj", &VAO_LegoAss, escalaModelAss, centreBaseModelAss);
